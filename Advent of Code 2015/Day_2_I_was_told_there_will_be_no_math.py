...
while True:
    ...
    if user_input == '':
        break
    list_of_input = user_input.split('x')
    side_a = int(list_of_input[0]) * int(list_of_input[1])
    side_b = int(list_of_input[0]) * int(list_of_input[2])
    side_c = int(list_of_input[1]) * int(list_of_input[2]) 
    smallest_area = min(side_a, side_b, side_c)

    result += 2 * side_a + 2 * side_b + 2 * side_c + smallest_area
...
# the required wrapping paper for each gift a little easier: find the surface area of the box, which is 
