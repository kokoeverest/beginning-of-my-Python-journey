...
while True:
    ...
    digits_input = [int(i) for i in list_of_input]

    smallest_sides = sorted(digits_input)
    smallest_side = smallest_sides[0]
    medium_side = smallest_sides[1]

    result += (2 * smallest_side + 2 * medium_side) + \
              (int(list_of_input[0]) * int(list_of_input[1]) * int(list_of_input[2]))
