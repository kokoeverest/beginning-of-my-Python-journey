...
while start_index < beggars:
    current_beg = 0
    for beggar in range(start_index, len(digits), beggars):
        current_beg += digits[beggar]

    sum_of_all_begs.append(current_beg)
    start_index += 1
print(sum_of_all_begs)
# You will receive 2 lines of input. On the first line, you will receive a single string of integers, separated
# by a comma and a space ", ". On the second line, you will receive a count of beggars. Your job is to print a
# list with the sum of what each beggar brings home, assuming they all take regular turns, from the first to the
