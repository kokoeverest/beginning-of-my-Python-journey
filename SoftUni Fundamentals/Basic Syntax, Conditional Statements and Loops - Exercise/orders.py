...
for order in range(orders):
    ...
    order_price = capsules_per_day * days * capsule_price
    if not 0.01 <= capsule_price <= 100.00 or not 1 <= days <= 31 or not 1 <= capsules_per_day <= 2000:
        continue
    else:
...
# each order. On the first line, you will receive integer N - the number of orders the shop will receive. For each \
# order, you will receive the following information:
...
# If you do not receive a correct order (one or more of the values are not in the given range), you should ignore it \
